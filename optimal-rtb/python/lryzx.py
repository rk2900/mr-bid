...
with open(sys.argv[3],'w') as outputR:
    # test for this round
    y = []
    yp = []
...
#    print str(round) + '\t' + str(auc) + '\t' + str(rmse)
    outputR.write( '(auc,rmse,cross_entropy) : ' + str(auc) + '\t' + str(rmse) + '\t' + str(cross_entropy))
    outputR.write('\n')
    outputR.write( 'time : ' + str( time.time()-beginTime) + ' seconds' )
